            ...
            if (!workspace) {
                ...
                await rp(getChannelUsersRequest).then( async (result) => {
                    ...
                    actualUsers = await Promise.all(actualUsers).then(result => {
                        return result.filter(user => user.user.is_bot == false)
                            .map(user => {
                                return {
                                    id: user.user.id,
                    ...
                    if (!status.ok) console.log("Error at postMessage request (onboarding) "+JSON.stringify(status))
                })
...
async function sendPrivateMessage(resp) {
    ...
    await rp(userRequest).then(async (result) => {
        if (result.ok){
            ...
            await admin.database().ref(`users/${teamId}/${userId}`).set(user)
                .then(async result => await rp(postMessageRequestOptions))
        }
        else console.log("Error processing request")
    })
